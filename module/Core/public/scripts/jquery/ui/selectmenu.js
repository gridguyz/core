...
$.widget( "ui.selectmenu", {
	...
	_create: function() {
		...
		this._on( document, {
			click: function( event ) {
				if ( this.isOpen && !$( event.target ).closest( "li.ui-state-disabled, li." + that.widgetBaseClass + "-optgroup, #" + this.ids.button ).length ) {
					this.close( event );
				}
			}
		});
	...
	_drawButton: function() {
		...
		this.buttonText = $( '<span />', {
				'class': this.widgetBaseClass + '-text' ,
		...
		this.buttonWrap = $( '<span />', {
				'class': this.widgetBaseClass + '-button'
			})
			.append( this.button )
	...
	_drawMenu: function() {
		...
		// wrap menu
		this.menuWrap = $( '<div />', {
				'class': this.widgetBaseClass + '-menu',
	...
	refresh: function() {
		...
		if ( options.length ) {
			...
			this.menu.toggleClass( "ui-menu-icons", !!this.options.icons );
			this.menuItems = this.menu.find( "li" ).not( '.' + this.widgetBaseClass + '-optgroup' );
	...
	_renderMenu: function( ul, items ) {
		...
		$.each( items, function( index, item ) {
			if ( item.optgroup !== currentOptgroup ) {
				$( '<li />', {
					'class': that.widgetBaseClass + '-optgroup' + ( item.element.parent( "optgroup" ).attr( "disabled" ) ? ' ui-state-disabled' : '' ),
					html: item.optgroup
				}).appendTo( ul );
	...
	_renderItem: function( ul, item) {
		...
		if ( this.options.icons ) {
			a.prepend( $( "<span />", {
				'class': "ui-icon " + ( icon || (
					String( this.options.iconsPrefix ) +
					item.value
						.toLowerCase()
						.replace( /[^a-z0-9_-]/g, "-" )
				) )
	...
	_toggleAttr: function(){
		...
		this.menuWrap.toggleClass( this.widgetBaseClass + '-open', this.isOpen );
